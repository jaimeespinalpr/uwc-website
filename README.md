...
- If the repo is inside a protected macOS folder (like `Documents`), the installer will automatically use a detached `screen` session (or `nohup` if `screen` is unavailable) instead of `launchd`.
